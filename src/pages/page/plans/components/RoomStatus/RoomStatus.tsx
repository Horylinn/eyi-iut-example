  ...
  return (
    <>
      <div className={`room-status room-status--${status}`}>
        <h3 className="room-status__title">
          <span>{name}</span>
          <div className={`room-status__title--${status}`}></div>
        </h3>
        <p className="room-status__info update">
          Mise à jour : {update || <div className="end">-</div>}
        </p>
        <p className="room-status__info">
          Température :{" "}
          {temperature ? (
            getRoomInfo("temperature", temperature)
          ) : (
        ...
        <p className="room-status__info">
          Humidité :{" "}
          {humidity ? (
            getRoomInfo("humidity", humidity)
          ) : (
            <div className="end">-</div>
          )}
        </p>
        <p className="room-status__info">
          CO2 : {co2 ? getRoomInfo("co2", co2) : <div className="end">-</div>}
        </p>
        <p className="room-status__info">
          Présence d'usager : {getRoomInfo("presence", presence)}
        </p>
        {status === "operational" ? (
